...
export default function ShowerHeroV2() {
  return (
    <section className="relative bg-bone pt-24 md:pt-28 pb-16 md:pb-20 overflow-hidden">
      {/* Soft water-mist wash behind */}
      <div className="absolute inset-0 -z-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-[60%] h-full bg-gradient-to-bl from-water/60 via-mist/40 to-transparent" />
      </div>

      <div className="relative mx-auto max-w-[1400px] px-5 md:px-10 grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-center">
        {/* LEFT: product image — big, anchors the above-fold */}
        <div className="lg:col-span-6 order-2 lg:order-1 relative">
          <motion.div
            initial={{ opacity: 0, scale: 0.97 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}
            className="relative aspect-[4/5] lg:aspect-[3/4] overflow-hidden rounded-sm bg-mist"
          >
            <Image
            ...
            />
            {/* Floating stat pill — visible in first viewport */}
            <motion.div
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.6, duration: 0.7 }}
              className="absolute top-5 left-5 bg-bone/95 backdrop-blur px-4 py-3 rounded-sm shadow-md shadow-ink/5"
            >
        ...
        {/* RIGHT: benefit-forward copy, proof, offer, CTA */}
        <div className="lg:col-span-6 order-1 lg:order-2 relative z-10">
          {/* Star rating strip — HIGH on the page */}
          <motion.div
          ...
          >
            ...
            Independently lab-tested. Installs in 90 seconds.
          </motion.p>

          {/* Price + offer block — VISIBLE above the fold */}
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.4, duration: 0.6 }}
            className="mt-8 p-5 md:p-6 bg-mist/70 rounded-sm border border-ink/10"
          >
